...
| 4 | ⚠️ Partiel | Cutoff de fiabilité BV fait (heuristique côté client + drapeau `fiable` baké dans `prep_bv.py`), repli via export. **Déféré** : agrégation arrondissement/grand quartier + mini-cartes de comparaison embarquées (dépend de la définition de la maille GA — question ouverte). |
