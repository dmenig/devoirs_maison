...
//
// ⚠ Les SEUILS et la DÉCOMPOSITION reposent sur des HYPOTHÈSES PROVISOIRES (taux de
// qualification, participation attendue) regroupées dans CARNET_HYP pour rester ajustables.
// À VALIDER par le PEE avant diffusion (cf. EVOLUTIONS.md, chantier 3).
const CARNET_HYP={
...
function carnetScenarios(b){ const H=CARNET_HYP;
...
function carnetCompo(o,b){
  ...
  const blocInsc=(o.abst||0)+(o.noninsc||0)+(o.malinsc||0);
  const inaccessibles=Math.max(0,b.elig-garanties-potentielles-blocInsc);
  const segs=[["Voix garanties",garanties,"#cf2e5b"],["Voix potentielles",potentielles,"#e6902e"],
    ["Abstention · non- et mal-inscription",blocInsc,"#8a8a8a"],["Voix inaccessibles",inaccessibles,"#3b6ea5"]];
